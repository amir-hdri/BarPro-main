...
def test_worker_heartbeats_endpoint():
    with patch("app.core.worker_heartbeat.worker_heartbeat_registry.snapshot", return_value={"task-1": {"status": "running"}}), patch(
...
def test_management_summary_endpoint():
    ...
    with patch("app.core.config.utcms_config.API_AUTH_MODE", "off"), patch(
        "app.api.routes.management.management_service.summary",
