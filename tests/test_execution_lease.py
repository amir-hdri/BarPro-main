...
async def test_orphan_detector(async_session):
    async with async_session() as session:
        ...
        job = WaybillJob(job_id="job-stale", idempotency_key="id-stale", client_id=1, status=TaskStatus.RUNNING.value, payload_json={})
        session.add(job)
    ...
    async with async_session() as session:
        ...
        reconcile_intent = (await session.exec(
            select(DispatchIntent).where(DispatchIntent.job_id == "job-stale").where(DispatchIntent.operation == "reconciliation")
        )).first()
