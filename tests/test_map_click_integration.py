...
from app.automation.map_click_selector import ClickLocation, ClickSelection, MapClickSelector, get_map_click_selector
...
class TestMapClickIntegration:
    ...
    def test_click_location_creation(self):
        ...
        location = ClickLocation(latitude=35.6892, longitude=51.3890, pixel_x=400, pixel_y=300, label="origin")
    ...
    def test_convert_click_to_location_data(self):
        ...
        click_loc = ClickLocation(latitude=35.6892, longitude=51.3890, pixel_x=400, pixel_y=300)

        # تبدیل به فرمت مورد انتظار LocationSelector
        location_data = {
            "province": "تهران",
            "city": "تهران",
            "coordinates": {"lat": click_loc.latitude, "lng": click_loc.longitude},
        }
    ...
    async def test_clear_markers(self):
        ...
        selector.selection.origin = ClickLocation(
            latitude=35.6892, longitude=51.3890, pixel_x=400, pixel_y=300, label="origin"
        )
    ...
    def test_get_selection_result(self):
        ...
        selector.selection.destination = ClickLocation(
            latitude=35.7000, longitude=51.4000, pixel_x=500, pixel_y=400, label="destination"
        )
    ...
    def test_geo_coordinate_compatibility(self):
        ...
        geo_coord = GeoCoordinate(latitude=click_loc.latitude, longitude=click_loc.longitude)
    ...
    async def test_full_selection_flow(self):
        ...
        selector.selection.selection_complete = True
        selector.selection.map_bounds = {"north": 35.8, "south": 35.6, "east": 51.5, "west": 51.3}
...
class TestDataFlow:
    ...
    def test_click_to_waybill_data(self):
        ...
        origin_click = ClickLocation(latitude=35.6892, longitude=51.3890, pixel_x=400, pixel_y=300)

        destination_click = ClickLocation(latitude=35.7000, longitude=51.4000, pixel_x=500, pixel_y=400)
    ...
    def test_coordinate_validation(self):
        ...
        for lat, lng in valid_coords:
            location = ClickLocation(latitude=lat, longitude=lng, pixel_x=0, pixel_y=0)
