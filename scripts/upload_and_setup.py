...
def build_archive():
    ...
    with tarfile.open(path, "w:gz") as tar:
        for r, dirs, files in os.walk(root):
            ...
            for f in files:
                if f in {".env", "backend.log", "celerybeat-schedule.db"}:
                    continue
                if any(f.endswith(e) for e in {".pyc", ".log", ".pid", ".tar.gz", ".zip", ".save", ".keras", ".pth"}):
                    continue
...
def main():
    ...
    with SCPClient(
        transport, progress=lambda f, s, t: print(f"\r  {int(s/t*100)}%" if t > 0 else "", end="", flush=True)
    ) as scp:
