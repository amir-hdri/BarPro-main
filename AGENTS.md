...
### Server-side Android / FakeTraveler direction (2026-09-15)

- **User-selected target, not deployed state:** FakeTraveler replaces the current
  GPS execution approach. BarPro orchestrates a virtual Android client on a Linux
  server; FakeTraveler supplies its location, and the official UTCMS Android app
  performs shipping actions. A physical phone is not a prerequisite.
- Target flow: `BarPro → Android Bridge → FakeTraveler → Android location →
  com.baarnameshahri → Squid → UTCMS`. FakeTraveler is not a replacement for
  Redroid, the official UTCMS application, tenant authorization, or reconciliation.
- The checked-out FakeTraveler package is **`cl.coders.faketraveler`**.
  `org.woheller69.mocklocation.UPDATE_LOCATION` is not its implemented broadcast
  contract. Its `geo:` activity input populates fields; it does not itself prove
  that a location has been applied. Verify Apply/Stop state and provider read-back.
- Current `/shipping/start` and `/shipping/finish` still use the Python mobile
  transport. That is the migration baseline, not the final target architecture.
  Do not add an automatic HTTP fallback after an Android action with unknown outcome.
- The first implemented bridge (`app/android_bridge/`) is opt-in observation only,
  launched on the Android server with an explicit Redroid ADB serial. It checks both
  APK packages and never clicks shipping controls or injects locations.
- Coordinate samples generated by FakeTraveler must retain provider/provenance
  metadata; do not relabel them as measured physical-device GPS. Keep planned
  waypoints separate from samples actually observed by the Android application.
- Preserve one active driver/execution per Android session, private ADB access,
  fail-closed egress, mutation fencing and read-back. A proxy setting or successful
  local test is not proof of WAF acceptance or successful shipping.
- Redroid boot, binder access, image/signature provenance and resource isolation
  remain deployment acceptance gates; the existing ban on `privileged: true`
  remains applicable. See the replacement plan for current evidence and milestones.
