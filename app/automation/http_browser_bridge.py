...
def validate_submission_coordinates(body: str) -> None:
    """Reject UpdateRegister requests that would submit fabricated coordinates."""
    values = dict(parse_qsl(body, keep_blank_values=True))
    required = ("sourceLatM", "sourceLonM", "destLatM", "destLonM")
    ...
    if missing or invalid:
        details = ", ".join((*missing, *invalid))
        raise WaybillError(f"مختصات واقعی برای ثبت بارنامه معتبر نیستند: {details}")
