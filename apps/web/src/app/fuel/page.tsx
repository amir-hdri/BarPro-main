...
export default function FuelInquiryPage() {
  ...
  // Timed Stepper states
  const [elapsedTime, setElapsedTime] = useState(0);
  
  // Lightbox / screenshot preview states
  const [lightboxOpen, setLightboxOpen] = useState(false);
  
  // Tabbed view for tables in modal
  const [modalTabIdx, setModalTabIdx] = useState(0);

  const pollingRef = useRef<NodeJS.Timeout | null>(null);
  const timerRef = useRef<NodeJS.Timeout | null>(null);

  // Load drivers and inquiry history
  ...
  // Polling logic for active inquiry
  ...
  // Compute stats from inquiries history
  const stats = useMemo(() => {
  ...
  const totalSecondsEst = 50;
  const progressPercent = Math.min((elapsedTime / totalSecondsEst) * 100, 98);
  ...
  return (
    <AuthGuard>
      <AppShell>
        <div className="mx-auto max-w-7xl px-3 py-4 sm:px-6 sm:py-8 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 mb-6 md:mb-8">
            {[
              ...
              <div key={idx} className="stat-card group relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1 space-y-6">
              <div className="rounded-3xl border border-white/10 bg-slate-950 p-6 shadow-xl backdrop-blur-md relative overflow-hidden">
                ...
                <div className="space-y-5 relative z-10">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-cyan-400 to-cyan-600 px-4 py-4 text-sm font-black text-white transition hover:opacity-90 active:scale-95 disabled:opacity-50 disabled:pointer-events-none shadow-[0_0_35px_rgba(6,182,212,0.3)]"
                  >
                    {submitting ? (
                      <>
              ...
              {/* Active Loading Progress Card (Timed Stepper) */}
                          ...
                          <div key={item.id} className="p-4 space-y-4">
                            <div className="flex items-start justify-between">
                              ...
                              <div>
                                ...
                                {item.status === 'failed' && (
                                  <span className="inline-flex items-center gap-1 rounded-lg bg-rose-500/10 border border-rose-500/20 px-2 py-0.5 text-[10px] font-bold text-rose-400">
                                    ناموفق
                                  </span>
                                )}
                                {item.status === 'processing' && (
                                  <span className="inline-flex items-center gap-1 rounded-lg bg-cyan-500/10 border border-cyan-500/20 px-2 py-0.5 text-[10px] font-bold text-cyan-400">
                                    در حال اجرا
                                  </span>
                                )}
                                {item.status === 'pending' && (
                                  <span className="inline-flex items-center gap-1 rounded-lg bg-slate-500/10 border border-slate-500/20 px-2 py-0.5 text-[10px] font-bold text-slate-400">
                                    در صف
                                  </span>
                                )}
                              </div>
        ...
        {/* Details Modal */}
        {selectedInquiry && (
          <div id="print-modal-portal" className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
            <div
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-5 mb-6 relative z-10 no-print">
                ...
                <button
                  ...
                  className="rounded-xl border border-white/10 bg-slate-900 p-2.5 text-slate-400 hover:text-white transition hover:scale-105 active:scale-95"
                >
              ...
              <div className="flex-1 overflow-y-auto space-y-8 pr-1 relative z-10 text-right" dir="rtl">
                {selectedInquiry.status === 'failed' ? (
                  ...
                  <>
                    ...
                    {/* Scraped Tables (Detailed quota history / Tabbed View) */}
                    {selectedInquiry.quota_data?.tables && selectedInquiry.quota_data.tables.length > 0 && (
                      <div className="space-y-4">
                        ...
                        {/* Tab Buttons (Hide when printing) */}
                                  ...
                                  <thead>
                                    <tr className="bg-white/5 text-slate-300 border-b border-white/5 font-bold print:bg-slate-50 print:text-black print:border-slate-300">
                                      {tbl.headers.map((hdr, hIdx) => (
                                        <th key={hIdx} className="px-4 py-3">{hdr}</th>
                                      ))}
                                    </tr>
                                  </thead>
                    ...
                    {selectedInquiry.screenshot_url && (
                      <div className="space-y-4 no-print">
                        ...
                        {/* Image display container with actions on hover */}
                        <div className="relative group overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-lg p-2 max-h-[400px] flex items-center justify-center">
                          ...
                          <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 no-print">
                            <button
                              onClick={() => setLightboxOpen(true)}
                              className="p-3 bg-slate-900 rounded-full border border-white/10 text-cyan-400 hover:bg-slate-800 transition"
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="p-3 bg-slate-900 rounded-full border border-white/10 text-slate-300 hover:bg-slate-800 transition"
                              title="دانلود تصویر"
                            >
