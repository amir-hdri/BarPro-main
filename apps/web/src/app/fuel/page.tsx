...
const getFriendlyErrorMessage = (errCode: string | undefined): string => {
  if (!errCode) return 'خطای نامشخص سامانه — لطفاً مجدداً تلاش نمایید.';
  switch (errCode) {
    case '101':
      return 'کد خطا: ۱۰۱ — عدم موفقیت در حل تصویر امنیتی کپچا. (راهکار: چند دقیقه دیگر مجدداً تلاش فرمایید)';
    case '102':
      return 'کد خطا: ۱۰۲ — عدم پاسخ‌دهی پرتال ملی UTCMS (سرور مبدا موقتاً در دسترس نیست)';
    case '103':
      return 'کد خطا: ۱۰۳ — تایم‌اوت ارتباط شبکه (پروکسی). سیستم به صورت خودکار مجدداً تلاش خواهد نمود';
    case '104':
      return 'کد خطا: ۱۰۴ — پلاک فعال یافت نشد یا رمز عبور سامانه ملی راننده اشتباه است (اطلاعات راننده را چک کنید)';
    case '100':
      return 'کد خطا: ۱۰۰ — خطای عمومی در دریافت سهمیه (پیشنهاد: بروزرسانی توکن راننده)';
    default:
      return `خطا در دریافت سهمیه: ${errCode}`;
  }
};
