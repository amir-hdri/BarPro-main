...
## ✅ 9. مدیریت تاب‌آوری و بازیابی تسک‌ها (Job Resilience)

### 9.1 مکانیزم بازیابی تسک‌های متوقف شده (Stuck Jobs)
**فایل**: `app/services/rpa_scheduler_service.py`

سیستم دارای یک تسک دوره‌ای (`phase1.scheduler.cleanup`) است که هر ۵ دقیقه اجرا شده و موارد زیر را بازیابی می‌کند:
- **وضعیت QUEUED**: تسک‌هایی که بیش از ۱۵ دقیقه در صف مانده و پردازش نشده‌اند.
- **وضعیت IN_PROGRESS**: تسک‌هایی که بیش از ۳۰ دقیقه در حال پردازش مانده‌اند (نشانه کرش کارگر یا قطع ارتباط).

✅ **اقدامات بازیابی**:
- تغییر وضعیت به `PENDING` برای تلاش مجدد خودکار.
- پاکسازی `celery_task_id` و `worker_id`.
- ثبت لاگ بازیابی در `WaybillTaskLog` برای شفافیت در داشبورد.

### 9.2 لایه محافظتی در سرویس‌های RPA
**فایل**: `app/services/rpa_submit_service.py`

متد `_process_job` مجهز به بلوک `try...except` سراسری است که در صورت بروز خطاهای پیش‌بینی نشده (Unhandled Exceptions) پس از شروع پردازش:
- تسک را بلافاصله به وضعیت `FAILED` تغییر می‌دهد.
- جزئیات خطا را در دیتابیس ثبت می‌کند.
- از باقی ماندن تسک در وضعیت "در حال پردازش" (`IN_PROGRESS`) جلوگیری می‌کند.

---

## ⚠️ 10. نکات قابل بهبود
