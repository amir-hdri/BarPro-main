...
## تلاش ثبت نظارتی دوم (job 109) — پورتال ۵۰۰ می‌دهد

- با فیکس کپچا، بات از کپچا گذشت و به GetCostSettings رسید؛ پورتال ۳ بار
  `HTTP 500` با متن «ارتباط با سرویس‌ها برقرار نیست» برگرداند (۵۷ بایت، text/plain).
- بات درست رفتار کرد: بدون mutation، وضعیت `otp_backoff` ثبت شد. ریشه فعلی اختلال
  سمت UTCMS است، نه کد ما. تلاش بعدی پس از پایداری پورتال + کد OTP اپراتور (در صورت نیاز).

## تلاش نظارتی سوم (job 109) — فرم لاگین روی پورتال نیست

- لاگین HTTP با وضعیت‌های بد + retry گذرا، سپس fallback مرورگری هم شکست:
  `auth_login_fields_not_found` (هر سه selector تهی). فرم ورود معتبر در URLهای
  شناخته‌شده وجود ندارد — پورتال امشب صفحات خراب/ناقص سرو می‌کند (سازگار با ۵۰۰های قبلی).
- هیچ mutation رخ نداد. جمع‌بندی هر سه تلاش: کد ما تا آخرین نقطه ممکن پیش رفت؛
  پورتال در لایه auth/document سرویس نمی‌دهد.
- درس persist: از `unknown` به `in_progress` یال مجاز نیست؛ در اجراهای نظارتی بعدی
  فقط در صورت تغییر واقعی وضعیت transition بزن، وگرنه فیلد + commit.
