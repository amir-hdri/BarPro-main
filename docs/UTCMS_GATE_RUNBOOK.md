...
2. **پیش‌بینی بازه شبانه (۱۷:۳۰ تا ۰۸:۰۰ به وقت تهران):**
   - مقادیر از `PREDICTED_OTP_REQUIRED_START_HOUR/MINUTE` و `PREDICTED_OTP_REQUIRED_END_HOUR/MINUTE` خوانده می‌شوند (پیش‌فرض ۱۷:۳۰ و ۰۸:۰۰).
...
3. **برخورد با OTP غیرمنتظره:**
   - اگر حین بازه ۱۷:۳۰ تا ۰۸:۰۰ چالش OTP مشاهده شود، کلید `rpa:gate:prediction_invalidated` ست شده و تا ۲۴ ساعت آینده فرضیه شبانه بی‌اعتبار می‌گردد.
