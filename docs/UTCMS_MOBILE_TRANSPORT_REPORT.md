...
### اصلاح معماری GPS و کلاینت (2026-09-12)

- BarPro به APK یا نصب روی تلفن راننده وابسته نیست؛ APK فقط مرجع استخراج
  قرارداد API است و ربات در Worker/Backend اجرا می‌شود.
- ثبت بارنامه و شروع/پایان حمل توسط کاربر اپراتور انجام می‌شود. رابط وب دیگر
  `navigator.geolocation` را فراخوانی نمی‌کند و از مختصات مبدأ/مقصد ثبت‌شده در
  payload بارنامه استفاده می‌کند.
- `waypoints` ساخته‌شده با interpolation فقط مسیر برنامه‌ریزی‌شده برای نمایش
  هستند. این نقاط هرگز به‌عنوان GPS واقعی یا در `gpsList` ارسال نمی‌شوند.
- در نبود دستگاه ردیاب یا منبع telemetry واقعی، ثبت N نقطه میانی روی UTCMS
  قابل ادعا نیست؛ endpoint میانی تا اثبات قرارداد زنده عمداً بسته می‌ماند.
- شروع/پایان فقط وقتی مختصات ارسالی را می‌پذیرد که با anchor همان بارنامه در
  تلورانس اعشاری کوچک تطبیق داشته باشد.

**تصمیم باقی‌مانده برای تکمیل مسیر N نقطه‌ای:** برای ثبت نقاط میانی واقعی باید
یک منبع telemetry مجاز (ردیاب مستقل یا کلاینتی که واقعاً همراه خودرو حرکت کند)
به endpoint رسمی `WS06_InsertBOLTrack` یا قرارداد معادل UTCMS متصل شود. تا زمان
ارائه این منبع و نمونه payload زنده، سیستم عمداً فقط anchor مبدأ/مقصد را ثبت
می‌کند و زمان/سرعت متوسط را به‌عنوان GPS جا نمی‌زند.
...
- **اصلاح:** در هر دو اندپوینت، فراخوانی مستقیم لاگین با `get_or_login_client()` جایگزین شد. این متد bearer token راننده را با TTL پیش‌فرض ۲۴۰ ثانیه و refresh token را با TTL پیش‌فرض ۷۰۰۰ ثانیه در Redis (`utcms:driver:token:{national_code}`) نگهداری می‌کند و در صورت انقضا ابتدا refresh را امتحان می‌کند. این کاهش لاگین تکراری است و تضمین حذف کامل 429 نیست.
