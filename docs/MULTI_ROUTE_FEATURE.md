...
> نسخه: Unreleased — آخرین هم‌ترازی: 2026-08-29 (`5d583a1`)
...
- فرم `/batches` موبایل واقعی فرستنده و گیرنده را با اعتبارسنجی `09xxxxxxxxx` می‌گیرد؛ هیچ شمارهٔ پیش‌فرض یا ساختگی تولید نمی‌شود.
- مقدار ارزش بار در UI و payload بر مبنای ریال نمایش و ارسال می‌شود.
- درخواست‌های مکان، راننده، فاصله و progress با `AbortController` لغوپذیرند؛ پاسخ قدیمی یا unmount شده state فرم را تغییر نمی‌دهد.
