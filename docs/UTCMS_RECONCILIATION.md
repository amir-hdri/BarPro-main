...
## ۴. قرارداد تصریح کد رهگیری (Tracking-First Acknowledgement)

از ۲۰۲۶-۰۹-۰۹، تطبیق History **فقط برای پیامدهای بدون کد یا مبهم** زمان‌بندی می‌شود:

- **کد رهگیری موجود (`tracking_received`):** تصریح فوری به اپراتور. Job در `unknown`
  با `requires_reconciliation=false` می‌ماند؛ تطبیق خودکار هرگز برای آن زمان‌بندی
  نمی‌شود. تنها مسیر بازبینی دستی، فراخوانی سرویس با `audit_only=True` است که
  صرفاً جهت اتصال مدرک History برای قانون سه‌شاهد انجام می‌شود و هرگز به
  resubmit منجر نمی‌شود.
- **پاسخ موفق‌نما بدون کد (`tracking_missing_history_required`):** مرز mutation
  عبور شده است؛ فقط تطبیق **فقط‌خواندنی** UTCMS History مجاز است با
  `reconciliation_mode='history_only'`.
- تاخیرهای محدودشده بدون تغییر می‌مانند: ۱۵ ثانیه، ۴۵ ثانیه، ۲ دقیقه، ۵ دقیقه.
- پس از اتمام پنجره بدون یافتن سند: `needs_review` + `submission_unconfirmed` —
  **هرگز ارسال مجدد خودکار**؛ تصمیم با اپراتور انسانی است.
- Dispatcher اینتنت‌های submit/reconciliation قدیمیِ jobهای دارای کد را لغو
  (`tracking_acknowledged`) می‌کند و scheduler این jobها را از صف submit خارج
  می‌کند؛ recovery کارهای گیرکرده نیز قرارداد `tracking_received` را حفظ می‌کند.
