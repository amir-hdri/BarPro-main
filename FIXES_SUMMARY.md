...
## ۲. رفع خطاهای ساخت Docker Image فرانت‌اند در CI/CD و سرور (Next.js Standalone Build)
...
*   **تغییرات اعمال شده:**
    *   داکرفایل فرانت‌اند به ساختار چندمرحله‌ای تبدیل شد: مرحله `deps` وابستگی‌ها را نصب می‌کند، مرحله `builder` دستور `npm run build` را اجرا می‌کند، و مرحله `runner` فقط خروجی standalone را اجرا می‌کند.
    *   با این تغییر، `bash manage.sh deploy` روی سرور می‌تواند فرانت‌اند را از یک checkout تمیز بسازد و دیگر نیازی به آپلود دستی `.next/standalone` نیست.
...
## ۷. آماده‌سازی نهایی برای آپلود و اجرای سرور (2026-07-08)

* **احراز هویت:** JWT به کوکی `httpOnly` منتقل شده است. در وضعیت فعلی HTTP باید `AUTH_COOKIE_SECURE=false` باشد؛ پس از نصب HTTPS مقدار آن باید `true` شود.
* **کپچای سوخت:** provider جدید `pytorch_fuel` و فایل‌های `fuel_captcha_crnn.pth` و `fuel_captcha_vocab.json` به دارایی‌های runtime اضافه شدند.
* **مایگریشن‌ها:** سر فعلی Alembic برابر `015_add_client_subscription_dates` است و دو migration جدید برای سال/ماه استعلام سوخت و تاریخ اشتراک مشتری اضافه شده‌اند.
* **Docker:** build بک‌اند و فرانت‌اند با Docker Compose با موفقیت بررسی شد. backend image شامل `persian_number_ocr.keras` و دارایی‌های fuel captcha است.
* **امنیت npm:** `npm audit --omit=dev` بدون آسیب‌پذیری است.
